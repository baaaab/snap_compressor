...
void usage(const char* argv0)
{
	...
	fprintf(stderr, "\tblock_size is the DCT size, larger values require lower quantisation_percent values to prevent overflows\n");
...
void encode(const char* inputFileName, uint32_t blockSize, float quantisationFactor, uint32_t binsToKeep)
{
	...
	FILE* roundedDctFh = NULL;
	FILE* roundedQuantisedDct = NULL;

	{
		char tmpFileName[1024];

		//snprintf(tmpFileName, sizeof(tmpFileName), "%s.roundedDCT", inputFileName);
		//roundedDctFh = fopen(tmpFileName, "w");
	...
	{
		//fwrite(magic, 1, 4, roundedDctFh);
		fwrite(magic, 1, 4, roundedQuantisedDct);

		//fwrite(&blockSize, 4, 1, roundedDctFh);
		fwrite(&blockSize, 4, 1, roundedQuantisedDct);

		//float dummyQuantisation = 1.0f;
		//fwrite(&dummyQuantisation, 4, 1, roundedDctFh);
	...
	while (!feof(fh))
	{
		if (fread(bytes.data(), 2, blockSize, fh) == blockSize)
		{
			...
			CDiscreteCosineTransform::DCT(floats, transformed);
			...
			//fwrite(transformedRounded.data(), 2, blockSize, roundedDctFh);
...
void decode(const char* inputFileName, const char* outputFileName)
{
	...
	//read headers (magic, blockSize, quantisationFactor, binsToKeep) all 4 bytes, LE
	{
		...
		fread(&quantisationFactor, 4, 1, fh);
	...
	while (!feof(fh))
	{
		if (fread(bytes.data(), 2, binsToKeep, fh) == binsToKeep)
		{
			for (uint32_t i = 0; i < blockSize; i++)
			{
				floats[i] = bytes[i];
			...
			CDiscreteCosineTransform::IDCT(floats, inverseTransformed);
			...
			/*for (uint32_t i = 0; i < blockSize; i++)
			 {
			 printf("%03d: (%3d, %3d) -> (%3.0f, %3.0f) -> (%3.0f, %3.0f) -> (%3d, %3d)\n",
			 i,
			 bytes[i].real(), bytes[i].imag(),
			 floats[i].real(), floats[i].imag(),
			 rounded[i].real(), rounded[i].imag(),
			 iBytes[i].real(), iBytes[i].imag()
			 );
			 }
			 printf("###################################\n");*/
